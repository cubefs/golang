...
func (check *Checker) declarePkgObj(ident *syntax.Name, obj Object, d *declInfo) {
	...
	if ident.Value == "fini" {
		check.error(ident, InvalidFiniDecl, "cannot declare fini - must be func")
		return
	}
	...
	for fileNo, file := range check.files {
		...
		for index, decl := range file.DeclList {
			...
			case *syntax.ImportDecl:
				...
				if name == "fini" {
					check.error(s, InvalidFiniDecl, "cannot import package as fini - fini must be a func")
					continue
				}
